...
namespace skc
{
    ...
    public:
        ...
        __forceinline operator std::basic_string<T>() const
        {
            skCrypter* self = const_cast<skCrypter*>(this);
            self->decrypt();
            return std::basic_string<T>(self->_storage, _size - 1);
        }
...
#define skCrypt_key(str, key1, key2) []() { \
    constexpr static auto crypted = skc::skCrypter \
        <sizeof(str) / sizeof(str[0]), key1, key2, skc::clean_type<decltype(str[0])>> \
        ((skc::clean_type<decltype(str[0])>*)str); \
    return crypted; }()
